...
<div class="row">
    <div class="col-md-6 mb-3">
        <button onclick="location.href=`{{ route('purchases.index') }}`" class="btn btn-sm btn-primary">Kembali</button>
    </div>
</div>
<div class="card">
    <div class="card-body py-4">
        <form id="purchaseForm" action="{{ route('purchases.store') }}" method="POST">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <select onchange="onChangeSupplier(this)" name="supplier_id" id="supplierId"
                                ...
                                <option {{ old('supplier_id')==$s->id ? 'selected' : '' }} data-supplier="{{
                    ...
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <input value="{{ old('order_date') }}" type="date" name="order_date" class="form-control"
                                required>
                        </div>
                    </div>
                    <div class="mb-3 row">
                        ...
                        <div class="col-sm-8">
                            <input value="{{ $helpers::formatToRupiah(old('total_price')) }}" type="text"
                                id="totalPrice" class="form-control" disabled>
                            <input value="{{ old('total_price') }}" type="hidden" name="total_price"
                                id="totalPriceHidden">
                        </div>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="mb-3 row d-flex align-items-center">
                        ...
                        <div class="col-sm-8">
                            <input value="{{ old('is_paid') }}" type="checkbox" id="isPaid" name="is_paid">
                        </div>
                    </div>
                </div>
            </div>
            <hr>
            <div class="my-3">
                <button type="button" id="addProduct" class="btn btn-primary mb-3" {{ session('request') ? ''
            ...
            <div class="table-responsive">
                <table class="table table-lg table-stripped" id="tabelDetail">
                    ...
                    <tbody id="tabelProdukBody">
                        ...
                        @foreach (session('request') as $index => $product)
                        <tr>
                            <td>
                                <input type="text" class="form-control input-sku" name="products[{{ $index }}][sku]"
                                    maxlength="20" placeholder="SKU" required value="{{ $product['sku'] }}">
                            </td>
                            <td>
                                <input type="text" class="form-control" name="products[{{ $index }}][name]"
                                    placeholder="Nama Barang" maxlength="30" required value="{{ $product['name'] }}">
                            </td>
                            <td>
                                <input type="number" class="form-control harga-beli"
                                    name="products[{{ $index }}][buy_price]" placeholder="Harga Beli" min="0" required
                                    value="{{ $product['buy_price'] }}">
                            </td>
                            <td>
                                <input type="number" class="form-control harga-jual"
                                    name="products[{{ $index }}][sell_price]" placeholder="Harga Jual" min="0" required
                                    value="{{ $product['sell_price'] }}">
                            </td>
                            <td>
                                <input type="number" class="form-control qty" name="products[{{ $index }}][quantity]"
                                    placeholder="Qty" value="{{ $product['quantity'] }}" min="1" required>
                            </td>
                            @php
                            $total = $product['buy_price'] * $product['quantity'];
                            @endphp
                            <td class="total-harga">Rp. {{ number_format($total, 0, ',', '.') }}</td>
                            <input type="hidden" name="products[{{ $index }}][total_price]" class="total-price-hidden"
                                value="{{ $total }}">
                            <td>
...
<script>
    let selectedProduct = [];
        ...
        addProduct.addEventListener('click', () => {
            ...
            // Fungsi update total dari harga beli * qty
            function updateTotal() {
                ...
                updateTotalKeseluruhan(); // update total semua produk
            }

            // Validasi max 10 digit untuk harga beli, harga jual, dan qty
            [hargaBeliInput, hargaJualInput, qtyInput].forEach(input => {
                input.addEventListener('input', function () {
                    this.value = this.value.replace(/\D/g, '').slice(0, 10); // Hanya angka, maksimal 10 digit
                    updateTotal();
                });
            ...
            // SKU dibatasi 20 karakter (sudah di HTML)
            const skuInput = tr.querySelector('.input-sku');
            skuInput.addEventListener('input', function () {
                this.value = this.value.slice(0, 20); // Hanya maksimal 20 karakter string
            });

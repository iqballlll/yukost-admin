        ...
        <form action="{{ route('sales.store') }}" method="POST" id="formPenjualan">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <select onchange="onChangeCustomer(this)" name="outlet_id" id="customerId" class="form-select"
                        ...
                        <option data-outlet="{{ json_encode($c) }}" value="{{ $c->id }}">{{ $c->outlet_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-md-4">
            ...
            <div class="row mb-5">
                ...
                <div class="col-md-4">
                    ...
                    <input type="hidden" name="total_price_after_discount" id="finalPriceHidden">
                </div>
            </div>
            ...
            <div class="my-3">
                <button id="buttonAddProduct" type="button" class="btn btn-primary mb-3" data-bs-toggle="modal"
                    data-bs-target="#modalProduk" disabled>
                    Tambah Produk
                </button>
                <br>
...
<script>
    let selectedProduct = [];
        ...
        document.addEventListener('DOMContentLoaded', rebindModalProductEvents);
                                ...
                                tr.innerHTML = `
                                                        ...
                                                        <td>
                                                           <input type="number" class="form-control qty-input" value="1" min="${prod.stock - prod.min_stock == 0 ? '' : '1'}"
                                                            max="${prod.stock - prod.min_stock}" style="width: 80px;"
        ...
        // Saat user mengubah kuantitas
        document.querySelectorAll('.qty-input').forEach(input => {
            input.addEventListener('input', debouncedUpdate);
        });

        // Saat user input diskon
        ...
        function rebindModalProductEvents() {
            // Rebind checkbox select all
            const toggleAllCheckbox = document.querySelector('input[type="checkbox"][onclick="toggleAll(this)"]');
            ...
            // Rebind tombol pilih produk
            const pilihProdukBtn = document.getElementById('btnPilihProduk');
            if (pilihProdukBtn) {
              
                pilihProdukBtn.onclick = function () {
                  
                    const checkedProducts = Array.from(document.querySelectorAll('.produk-checkbox:checked')).map(cb => {
                  ...
                  if (checkedProducts.length === 0) {
                    bootstrap.Modal.getInstance(document.querySelector('#modalProduk')).hide()
                    alert('Pilih minimal satu produk terlebih dahulu!');
                    return;
                    }
